...
// Lockout duration in seconds (4 hours)
$lockoutDuration = 3 * 60 * 60;
        ...
        if (isset($_SESSION['lockout'][$username]) && time() < $_SESSION['lockout'][$username]) {
            ...
            if ($captcha === $_SESSION['captcha']) {  // Captcha verification passed
                ...
                if (isset($validAccounts[$username]) && $validAccounts[$username] === $password) {
                    ...
                    if (isset($_SESSION['lockout'][$username])) {
                        $remainingTime = $_SESSION['lockout'][$username] - time();
                        if ($remainingTime > 0) {
                            $error = 'Account locked. Please try again after ' . gmdate('H:i:s', $remainingTime);
                        } else {
                            unset($_SESSION['lockout'][$username]); // Unlock the account
                        }
                    }
